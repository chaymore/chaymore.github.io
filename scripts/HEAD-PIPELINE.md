...
4. Run `prepare-head.py MODEL_DIRECTORY public/head`. It expects `head.obj` plus `unpacked/0/*tex*` from this reconstruction. The crop above y=0.575, rotation, origin, and lip seam are specific to Caleb’s scan. Inspect a new scan before reusing them.
5. The script samples the face more densely than the shoulders, removes triangles crossing the lip seam, thins the lower chest, and emits quantized version-2 bust assets. Stable seeds preserve sampling across rebuilds of the same mesh.
6. Build the website and visually check front, profiles, back, mobile, both rendering modes, and the speaking preview.
